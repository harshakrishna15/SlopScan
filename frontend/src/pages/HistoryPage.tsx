        ...
        <section className="glass-panel fade-up mx-auto w-full max-w-5xl rounded-3xl p-5 md:p-6">
          ...
          {items.length === 0 ? (
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              {items.map((item) => {
                const { Icon } = getUnifiedCategory(item.product?.categories || '', item.product_name);
                return (
                  <button
                    key={item.id}
                    onClick={() =>
                      navigate(`/product/${item.product_code}`, {
                        state: { product: item.product, capturedImage: item.captured_image || undefined },
                      })
                    }
                  ...
                  >
                    <div className="flex h-24 w-24 flex-shrink-0 items-center justify-center rounded-xl border border-[var(--line-soft)] bg-[var(--surface-100)]">
                      <Icon className="h-9 w-9 text-[var(--ink-700)]" />
                    </div>
